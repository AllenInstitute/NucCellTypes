...
for (samp.set in c("nuc", "cell", "snap25")[2]) {
  for (read.set in c("intron_exon", "exon", "intron", "varIE_clE", "varE_clIE")[4:5]) {
    ...
    MakeShinyFeatherDb(root.path = root.path, dat.path = dat.path, dat.file = dat.fn, 
                       ...
                       anno.cols = c("roi", "external_donor_name", "gender", 
                                     "cell_prep_type", "cre_line", 
                                     "transgenic_recombinase", "recombinase", 
    ...
    if (samp.set == "nuc") {
      load(paste0(input.path.cell.nuc, "/fpkm_Mouse_Star_VISp_L5_Nuc.Rdata"))
    } else {
