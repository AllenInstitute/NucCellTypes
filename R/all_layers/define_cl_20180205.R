...
batch.date <- paste0("20180205_VISp_nuc")
run.num <- 1
...
#### Zizhen script ####
source("//allen/programs/celltypes/workgroups/0285/Personal_folders/Zizhen/My_R/sparse_cluster/cluster.new.R")
source("//allen/programs/celltypes/workgroups/0285/Personal_folders/Zizhen/My_R/sparse_cluster/vg.R")
source("//allen/programs/celltypes/workgroups/0285/Personal_folders/Zizhen/My_R/sparse_cluster/de.genes.R")
source("//allen/programs/celltypes/workgroups/0285/Personal_folders/Zizhen/My_R/heatmap.R")


co.ratio <- read.csv(file = "//allen/programs/celltypes/workgroups/hct/CT_clustering/mouse_cell_vs_nuc/cache/20180205_VISp_nuc/1/cl.cons.csv.gz", row.names = 1)
co.ratio <- as.matrix(co.ratio)
colnames(co.ratio) <- row.names(co.ratio)
load("//allen/programs/celltypes/workgroups/hct/CT_clustering/mouse_cell_vs_nuc/data/20180205_VISp_nuc_iter_cl_data.rda")

cl = recursive_split(co.ratio, nbt.data, select.cells=colnames(nbt.data), 
                     all.col=t(samp.dat), prefix="all", 
                     q1.th=0.5, q.diff=0.7, min.padj=150, min.cells=4)
cl.result = displayResult(cl, nbt.data, q1.th=0.4, q.diff.th=0.7)
de.genes= cl.result$de.genes
select.markers=cl.result$markers

cl.merge= mergeCl.v4(norm.dat=nbt.data, cl=cl, rd.dat=t(nbt.data[select.markers,]),  
                     q1.th=0.5, q.diff=0.7, min.cells=4, min.padj=150)
cl2 = cl.merge$cl

cl.merge2= reassign_co_ratio(co.ratio, cl2, confusion.th=0.5)
cl3 = cl.merge2$cl
write.csv(cl3, file = "//allen/programs/celltypes/workgroups/hct/CT_clustering/mouse_cell_vs_nuc/cache/20180205_VISp_nuc/1/cl_merge.csv")
