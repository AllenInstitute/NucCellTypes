...
all.paths <- paste0(root.path,
                    c("/rnaseqanalysis/shiny/facs_seq/mouse_V1_ALM_20170905_exons_introns/", 
                      "/hct/CT_clustering/mouse_cell_vs_nuc/shinydb/20171211_VISp_nuc_1/",
                      "/hct/CT_clustering/mouse_cell/shinydb/20170908_V1_4671_nuc_match_1/",
                      ...
                      "/hct/CT_clustering/human_nuc/shinydb/20170615_STAR_MTG_1/",
                      "/hct/CT_clustering/human_nuc/shinydb/20171023_VISp_3/"))
names(all.paths) <- c("Mouse_V1_ALM", "Mouse_V1_Nuc", "Mouse_V1_Cell",
                      "Mouse_L5_Nuc", "Mouse_L5_Cell", 
                      "Mouse_V1_Tasic",
                      "Human_MTG", "Human_VISp")
paths <- all.paths[c("Mouse_V1_Nuc", "Mouse_V1_ALM")]
...
for (dat1 in names(paths)) {
  ...
  if (dat1 == "Mouse_V1_ALM") {
  ...
  } else if (grepl("Mouse_V1_Cell", dat1)) {
    colnames(prop1) <- unique(anno[[dat1]]$cluster_label)
    # keep.cl <- unique(anno[[dat1]]$cluster_label)
    keep.cl <- unlist(dendrapply(dend[[dat1]], function(x) if (is.leaf(x)) attr(x, "label")))
    # keep.cl <- unique(anno[[dat1]][anno[[dat1]]$cluster_type_label %in% c("inh", "exc", "glia"), c("cluster_label")])
    anno.cl1 <- data.frame(cluster_label = unique(anno[[dat1]][, c("cluster_label")]))
  } else if (grepl("Mouse_V1_Nuc", dat1)) {
    colnames(prop1) <- unique(anno[[dat1]]$cluster_label)
    # keep.cl <- unique(anno[[dat1]]$cluster_label)
    keep.cl <- unlist(dendrapply(dend[[dat1]], function(x) if (is.leaf(x)) attr(x, "label")))
    # keep.cl <- unique(anno[[dat1]][anno[[dat1]]$cluster_type_label %in% c("inh", "exc", "glia"), c("cluster_label")])
    ...
    anno.cl1 <- data.frame(cluster_label = unique(anno[[dat1]][, c("cluster_label")]))
  } else if (grepl("Human_MTG", dat1)) {
    keep.cl <- unlist(dendrapply(dend[[dat1]], function(x) if (is.leaf(x)) attr(x, "label")))
    # keep.cl <- intersect(keep.cl, unique(anno[[dat1]]$cluster_label[anno[[dat1]]$cluster_type_label %in% c("inh")]))
    anno.cl1 <- unique(anno[[dat1]][, c("cluster_label", "cluster_type_label")])
    confusion.score <- tapply(anno[[dat1]]$cc.min.diff_label, anno[[dat1]]$cluster_label, median)
    anno.cl1$confusion_score <- confusion.score[match(anno.cl1$cluster_label, names(confusion.score))]
  } else if (grepl("Human_VISp", dat1)) {
    keep.cl <- unlist(dendrapply(dend[[dat1]], function(x) if (is.leaf(x)) attr(x, "label")))
    # keep.cl <- intersect(keep.cl, unique(anno[[dat1]]$cluster_label[anno[[dat1]]$cluster_type_label %in% c("inh")]))
    anno.cl1 <- unique(anno[[dat1]][, c("cluster_label", "cluster_type_label")])
  }
...
```{r plot-dendrograms, eval = FALSE}
for (dat1 in names(paths)) {
  pdf(file = paste0("../../output/all_layers/", dat1, "_tree.pdf"), 
      width = 4, height = 4)
...
```{r plot-cor-heatmap, fig.width = 12, fig.height = 8}
...
g1 <- ggplot(cor.ml, aes(x = Var2, y = Var1, fill = value)) +
  ...
  ylab(names(paths)[1])
plot(g1)
ggsave(g1, filename = paste0("../../output/all_layers/", names(paths)[1],
...
```{r find-cl-homol, fig.width = 14, fig.height = 8}
...
cl.match <- cbind(rownames(cor.m2)[apply(cor.m2, 2, which.max)], 
                  colnames(cor.m2))
good.match <- which(apply(cor.m2, 2, max) >= 4 & apply(cor.diffm, 2, max) > 0.0)
cl.to.plot <- cl.match[good.match, ]
cl.to.plot.lab <- sapply(cl.to.plot[, 1], function(x) strsplit(x, "_")[[1]][4])
...
if (nrow(cl.to.plot) > 0) {
  ...
  plot(g1)
  ggsave(g1, filename = "../../output/all_layers/cl_prop_scatter.pdf", 
         width = 3, height = 3)
}
...
write.csv(nuc.cell.prop[order(nuc.cell.prop$cluster_pair), ], 
          file = "../../output/all_layers/nuc_cell_cl_prop_table.csv",
          row.names = FALSE)
```
...
if (nrow(cl.to.plot) > 0) {
  ...
  pdf(file = "../../output/all_layers/nuc_cell_cl_match_cor.pdf", 
      width = 8, height = 10)
  par(mfrow = c(ceiling(sqrt(nrow(cl.to.plot))), ceiling(sqrt(nrow(cl.to.plot)))))
  cl1.labs <- paste0("N", 1:nrow(cl.to.plot), "_", 
                     sapply(cl.to.plot[, 1], function(x) strsplit(x, "_")[[1]][4]))
  cl2.labs <- paste0("C", 1:nrow(cl.to.plot), "_", 
