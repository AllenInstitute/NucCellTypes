...
setwd("//allen/programs/celltypes/workgroups/mct-t200/Microscopy/Soma_vs_nuclei/Rbp4/current/after_edits/")
...
```{r combine-dat}
area.dat <- rbind(cbind(cre = "rbp4", subset(dat_rbp4, s_area > n_area, 
                                             select = c("n_area", "s_area", "nuc_soma_ratio"))),
                  cbind(cre = "nr5a1", subset(dat_nr5a1, s_area > n_area, 
                                             select = c("n_area", "s_area", "nuc_soma_ratio"))),
                  cbind(cre = "scnn1a_tg3", subset(dat_scnn1a, s_area > n_area, 
                                               select = c("n_area", "s_area", "nuc_soma_ratio"))),
                  cbind(cre = "scnn1a_tg2", subset(dat_scnn1a_tg2, s_area > n_area, 
                                               select = c("n_area", "s_area", "nuc_soma_ratio"))))

# Unit conversion
# Scnn1a-Tg2 1um per unit; other Cre-lines 0.22282 um per unit
area.dat$n_area[area.dat$cre != "scnn1a_tg2"] <- 0.22282^2 * area.dat$n_area[area.dat$cre != "scnn1a_tg2"]
area.dat$s_area[area.dat$cre != "scnn1a_tg2"] <- 0.22282^2 * area.dat$s_area[area.dat$cre != "scnn1a_tg2"]
...
g.hist <- ggplot(area.dat, aes(x = nuc_soma_ratio^(3/2), fill = cre)) +
...
g1 <- ggplot(area.dat, aes(x = s_area^(3/2), y = n_area^(3/2), 
                           color = cre, fill = cre)) +
