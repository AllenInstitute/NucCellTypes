...
# knitr::opts_knit$set(eval.opts = c('eval', 'echo', 'fig.height', 'fig.width'))
...
# Osnat (5/2017); Yanai 2005 Bioinformatics
...
for (dat1 in names(paths)) {
  ...
  tau[[dat1]] <- apply(prop[[dat1]], 1, calc_tau)
}
...
keep.genes <- which(cnt[[1]] < cnt1.max & cnt[[2]] < cnt2.max & 
                      cnt[[1]] > 0 & cnt[[2]] > 0 & 
                      (beta[[1]] > 0.3 | tau[[1]] > 0.9) & 
                      (beta[[2]] > 0.3 | tau[[2]] > 0.9))
...
# ph1 <- pheatmap(cor.m, cluster_cols = FALSE, cluster_rows = FALSE)
...
if (nrow(cl.to.plot) > 0) {
  ...
  for (i in 1:nrow(cl.to.plot)) {
    ...
    # tau[[1]][keep.genes] > 0.85 & tau[[2]][keep.genes] > 0.85
...
print(shared.marker.genes)

```


### Compare marker scores
```{r compare-marker-scores, fig.width = 4, fig.height = 4, eval = FALSE}
set1 <- 1:nrow(cor.m)
set2 <- 1:ncol(cor.m)
cor.subset <- cor.m2[set1, set2]
cl.weight <- ifelse(as.numeric(cor.subset) < 0, 0, as.numeric(cor.subset))
cl.weight <- cl.weight / max(cl.weight)

gene.cor <- NULL
prop.diff.df <- NULL
for (gene1 in names(keep.genes)) {
  prop1 <- as.matrix(prop.subset[[1]][gene1, set1])
  prop2 <- as.matrix(prop.subset[[2]][gene1, set2])
  prop.df <- expand.grid(prop1, prop2)
  prop.diff <- (prop.df[, 1] - prop.df[, 2])
  prop.diff.df <- rbind(prop.diff.df, prop.diff)
  
  cor1 <- corr(prop.df, w = cl.weight)
  gene.cor <- c(gene.cor, cor1)
  if (gene1 %in% c("GAD1", "PDGFRA", "NPY2R")) {
    plot(prop.df, col = grey(1 - cl.weight), cex = 2*cl.weight, pch = 19, 
         main = gene1, xlab = names(prop.subset)[1], ylab = names(prop.subset)[2])
  }
}
names(gene.cor) <- names(keep.genes)
rownames(prop.diff.df) <- names(keep.genes)

score.df <- data.frame(score1 = beta[[1]][keep.genes], 
                       score2 = beta[[2]][keep.genes], 
                       gene.cor)
score.df$min_score <- apply(score.df[, c("score1", "score2")], 1, min) 
score.df$score_diff <- score.df$min_score - score.df$gene.cor
pairs(score.df)
cor(score.df)

plot(gene.cor ~ min_score, score.df)

head(score.df[order(score.df$score_diff, decreasing = TRUE), ], 50)
subset(score.df, min_score > 0.4 & gene.cor < 0.2)
subset(score.df, gene.cor > 0.9)
