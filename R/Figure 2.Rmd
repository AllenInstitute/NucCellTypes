---
title: "Nuclei vs Cells - Figure 2"
...
load("../data/map_nuc_to_cells.rda")

# Load gene lengths
gene.len <- read.csv(file = "../data/mouse_GRCm38_gene_len.csv")
...
```{r Figure_2B-single-cell-detection, message=FALSE, warning=FALSE}
...
write.csv(cell.genes, file="../output/cell.genes_gt1.csv", row.names=FALSE)
write.csv(nuc.genes, file="../output/nuc.genes_gt1.csv", row.names=FALSE)

write.csv(genes$Nuclei, file="../output/nuc.all.genes_gt1.csv", row.names=FALSE)
write.csv(genes$Cells, file="../output/cell.all.genes_gt1.csv", row.names=FALSE)
...
for (i in 1:nlevels(samp.subset$Samples)) {
  ...
  ggsave(g1, width = 3, height = 3, dpi = 600,
         filename = paste0("../output/", method1, "_pair.png"))
}
...
ggsave(g0, width = 4, height = 3, 
       filename = "../output/nuc_vs_cell_pw_cor.pdf")
...
for (split.v4 in c(FALSE, TRUE, FALSE)) {  # 
  ...
  # write.csv(gene.prop, file = "../output/gene.prop.csv")
...
# Save list of genes
out.fn1 <- "../output/Cell_vs_nuc_gene_detection.csv"
...
out.fn2 <- "../output/Cell_vs_nuc_dex_genes_introns_exons.csv"
...
```{r Figure_2C-gene-prop-vs-dex, message = FALSE, warning = FALSE, fig.width = 5, fig.height = 4}
...
ggsave(g5, width = 4.25, height = 3, 
       filename = "../output/nuc_vs_cell_prop_scatter.pdf")
...
ggsave(g2, width = 4.25, height = 3, 
       filename = "../output/nuc_vs_cell_dex_volcano.pdf")
...
# Prop vs. expr
g6 <- ggplot(gene.prop.dex, aes(Cell_expr, Cell_prop)) +
  geom_hline(yintercept = 0) +
  stat_bin_hex(bins = 50) +
  scale_fill_gradientn(colours=pal.spectral, trans="log10", name="# Genes") +
  geom_smooth(data = prop.exp, aes(Cell_expr, exprq), 
...
```{r Figure_2D-gene-distrib-cellvsnuc, fig.width = 4.25, fig.height = 4}
...
ggsave(expr.violins[["nuc_vs_wc"]], width = 3.25, height = 4,
       filename = paste0("../output/nuc_vs_wc_violin.pdf"))

ggsave(expr.violins[["wc_vs_nuc"]], width = 3.25, height = 4,
       filename = paste0("../output/wc_vs_nuc_violin.pdf"))
...
ggsave(g1, width = 5, height = 3, dpi = 600,
         filename = paste0("../output/gene_length_vs_nuc_enrichment.png"))


```

```{r dropout-rate}
gene.prop.l <- rbind(data.frame(Sample = "Cell", 
                                Expression = gene.prop.dex$AveExpr, 
                                Dropout = 1 - gene.prop.dex$Cell_prop),
                        data.frame(Sample = "Nucleus", 
                                Expression = gene.prop.dex$AveExpr, 
                                Dropout = 1 - gene.prop.dex$Nucleus_prop))
gene.prop.l <- gene.prop.l[order(gene.prop.l$Expression), ]

g.dropout <- ggplot(gene.prop.l, aes(x = Expression, y = Dropout, color = Sample)) +
  # geom_vline(xintercept = 0, size = 1, color = "grey") +
  geom_point(alpha = 0.02) +
  geom_smooth(size = 2, se = FALSE) +
  xlab("Mean expression (log2 CPM + 1)") +
  ylab("Gene dropout (proportion of cells/nuclei with zero expression)") +
  # scale_color_gradientn(colors = grey.colors(10, 0.9, 0, gamma = 1)) +
  scale_color_manual(values = brewer.pal(3, "Set1")[2:1]) +
  xlim(0, 12) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
plot(g.dropout)
ggsave(g.dropout, width = 6, height = 4, dpi = 600,
       filename = "../output/nuc_vs_cell_gene_dropout.png")
...
out.fn3 <- "../output/Cell_vs_nuc_dex_genes_exons_only.csv"
...
```{r Figure_S2B-compare-dex-byreads, fig.width = 8, fig.height=4}
...
pdf(file = paste0("../output/gene_length_ecdf.pdf"),
    width = 8, height = 4)
...
pdf( file="../output/nuc_vs_cell_go_enrich_revigo_treemaps.pdf", width=7, height=4) # width and height are in inches
