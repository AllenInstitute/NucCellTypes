...
nuc.ratio.df$depth_from_pia_bin <- cut(nuc.ratio.df$depth_from_pia, 
...
g.prop.v.depth <- ggplot(nuc.ratio.df, aes(y = -depth_from_pia, x = nuc.vol.ratio,
  ...
  scale_size_area() +
  xlab("Nuclear volume proportion") +
  ylab(expression(paste("Depth from pia (", mu*m, ")"))) +
  scale_color_gradient(low = "grey80", high = "black") +
  guides(size = guide_legend(title = expression(paste("Soma volume (", mu*m^3, ")")))) +
...
g.prop.hist <- ggplot(nuc.ratio.df, aes(x = nuc.vol.ratio)) +
  ...
  theme(panel.grid = element_blank())
plot(g.prop.hist)
ggsave(g.prop.hist, file = "../output/nuc_prop_hist.pdf", 
       height = 4, width = 6)


g.prop.hist2 <- ggplot(nuc.ratio.df, aes(x = depth_from_pia_bin, y = nuc.vol.ratio)) +
  geom_boxplot() +
  xlab(expression(paste("Depth from pia (", mu*m, ")"))) +
  ylab("Nuclear volume proportion") +
  theme_bw(base_size = 18) +
  theme(panel.grid = element_blank())
plot(g.prop.hist2)
