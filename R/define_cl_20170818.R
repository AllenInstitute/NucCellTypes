...
for (samp.set in c("nuc", "cell", "snap25")[2]) {
  for (read.set in c("intron_exon", "exon", "intron", "varIE_clE", "varE_clIE")[4:5]) {
    ...
    batch.date <- paste0("20170818_VISp_L5_", batch.ext)
    print(batch.date)
    run.num <- 1
    ...
    for (run1 in 1:2) {
      ...
      samp.dat$cluster <- as.factor(paste0("cl", cl.df$x[match(rownames(samp.dat), cl.df$cellid)]))
      ...
      neun.pos.cnt <- tapply(samp.dat$cluster, samp.dat$cluster, function(x) return(1))
      neun.thresh <- 0.5
      
      
      samp.dat$reads_aligned_to_mrna_gdna <- samp.dat$reads_aligned_to_mrna + samp.dat$reads_aligned_to_genome_only
      ...
      pdf(file = paste0(analysis.path, "/cluster_marker_check.pdf"), 
          width = 16, height = 4)
      barplot(t(expr.cnt), ylim = c(0, 1), beside = TRUE, las = 2, 
              legend = colnames(expr.cnt), 
              main = "Prop. of nuclei with expression > 1")
      abline(h = 0.5)
      dev.off()
      
      expr.outlier <- neun.pos.cnt > neun.thresh & expr.cnt[, "SNAP25"] < 0.5 | 
