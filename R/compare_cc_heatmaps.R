...
base.path <- "//allen/programs/celltypes/workgroups/hct/CT_clustering/mouse_cell_vs_nuc/"

# Load nuc/cell matching
nuc.ids <- scan("analysis/match_nuc_cell/nuc_ids.txt", "character")
cell.ids <- scan("analysis/match_nuc_cell/cell_ids.txt", "character")
...
names(dirs) <- sets
paths <- paste0(base.path, "cache/20170818_VISp_L5_", dirs)
names(paths) <- sets
shiny.paths <- paste0(base.path, "shinydb/20170818_VISp_L5_", dirs)
names(shiny.paths) <- sets
...
anno.list <- list()
for (set1 in sets) {
  anno.list[[set1]] <- as.data.frame(read_feather(paste0(shiny.paths[set1], "_1/anno.feather")))
  # cl.df <- read.csv(file = paste0(paths[set1], "/1/cl_merge.csv"),
  #                   row.names = 1)
  cc1 <- read.csv(file = paste0(paths[set1], "/1/cl.cons.csv.gz"),
                  row.names = 1)
...
for (sample.type1 in c("nuc", "cell")) {
  for (set1 in sets[grep(sample.type1, sets)]) {
    ...
    png(file = paste0("analysis/match_nuc_cell/cc_ordered/", set1, ".png"), 
...
#### Within/between co-clustering - all comparisons ####
cc.stats <- data.frame()
for (sample.type1 in c("nuc", "cell")) {
  for (set1 in sets[grep(sample.type1, sets)]) {
...
for (cc.metric in c("cc.mean.within", 
  ...
  for (pair1 in unique(anno.nuc.cell$cluster_pair)) {
    ...
    coef1 <- t(coef(summary(lm1))["sample.typecell", ])
...
ggsave(g2a, width = 6, height = 4, 
       filename = "analysis/nuc_vs_cell_cc.mean.within.pdf")
...
ggsave(g2b, width = 6, height = 4, 
       filename = "analysis/nuc_vs_cell_cc.mean.max.between.pdf")
...
ggsave(g2c, width = 6, height = 4, 
       filename = "analysis/nuc_vs_cell_cc.mean.min.diff.pdf")
